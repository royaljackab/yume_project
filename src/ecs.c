...
{
    /***
     * Initialise un ECS avec la taille des components
     * 
     * Il faut passer la taille de chaque composante dans l'ordre
     * de stockage souhaité. 
     * Le maximum nombre de composante est INITIAL_CAPACITY
     * -> Possibilité d'augmenter en ajoutant bitmask + bit switch
     */
    uint32_t i;
    va_list ap;
    size_t sizes[32];
...
Entity ecs_create()
{
    ...
    if (state.entity_pool->count > 0)
        id = *(uint32_t*)Stack_pop(state.entity_pool);
    else {
...
{
    return (state.entity_store.mask_array[entity_id] & (1 << component_id)) != 0;
}
...
void ecs_kill(uint32_t entity_id)
{
    if ( (state.entity_store.flag_array[entity_id] & ENTITY_FLAG_ALIVE) != 0) {
        ...
        Stack_push(state.entity_pool, &entity_id);
    }
}

QueryResult *ecs_query(uint32_t n, ...)
{
    va_list ap;
    uint32_t i, mask = 0;
    ...
    for (i = 0; i < state.entity_store.count; i++) {
        if ((state.entity_store.flag_array[i] & ENTITY_FLAG_ALIVE) && mask == (state.entity_store.mask_array[i] & mask) != 0) {
