...
#define DRAW_KEYBINDS(buttonID, text)                                          \
  button_draw_button_text(&ctx->button, buttonID, text, 20, WHITE)
...
GameState state_menu_keybinds = {.init = state_menu_keybinds_init,
...
// NOTE: cette est beaucoup plus TUFF que tu ne le seras jamais
void button_get_key(const char *target_name, KeyboardKey *key_target) {
  ...
  while (!WindowShouldClose() && !key_assigned) {
    ...
    DrawText(TextFormat("Press the new key for '%s'", target_name), 120, 420,
             20, WHITE);
