...
ObjID obj =
    CreateLaser(bossX, bossY, 180 + 105 * dir, 400, 15, 120, BALL_M_BLACK, 30);
ObjMove_AddPattern(obj, 30, NO_CHANGE, NO_CHANGE, NO_CHANGE, NO_CHANGE,
                   0.5 * dir);
End_Task;
...
for (int i = 0; i < 5; i++) {
  ...
  ObjMove_AddPattern(laser, 40, NO_CHANGE, angleT + 20, NO_CHANGE, NO_CHANGE,
                     NO_CHANGE);
  angleT += 360.0 / 5;
}
End_Task;
...
while (true) {
  ObjMove_SetDestAtFrame(bossID, GetRandomValue(30, 500),
                         200 + GetRandomValue(-20, 50), 60);
  wait(360);
}
End_Task;

Define_Task(moonlight_task, NO_PARAMS, int count; void *ringState;
            void *laserState; void *moveState; void *loserState;);
ctx->ringState = NULL;
ctx->moveState = NULL;
ctx->laserState = NULL;
ctx->loserState = NULL;
